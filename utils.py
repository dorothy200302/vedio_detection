...
def bt_embedding_from_prediction_guard(text: str, model_name: str = "BAAI/bge-large-zh-v1.5") -> List[float]:
    """使用预训练模型生成文本嵌入"""
    try:
        # 加载模型和分词器
...
def process_video_metadata(video_path: str) -> Dict:
    """处理视频元数据"""
    try:
        cap = cv2.VideoCapture(video_path)
        metadata = {
